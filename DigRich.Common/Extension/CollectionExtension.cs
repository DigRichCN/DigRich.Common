...
namespace DigRich.Common.Extension
{
    ...
    public static partial class CollectionExtension
    {
        ...
        /// <returns></returns>
        private static List<T> GetRandomList<T>(this IList<T> list)
        {
            return Enumerable.Range(0, list.Count).OrderBy(_ => SecurityHelper.Random.Next(list.Count)).Select(i => list[i]).ToList();
        }
    }
}
